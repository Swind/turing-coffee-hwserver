...
class MAX31865(TemperatureSensor):
    ...
    """Reading Temperature from the MAX31865 with GPIO using
       ...
       Both the quadratic formula using Callendar-Van Dusen equation (ignoring
       the 3rd and 4th degree parts of the polynomial) and the straight line
       approx.
       temperature is calculated with the quadratic formula one being the most
       accurate.
    """

    def __init__(self, csPin=29, misoPin=33, mosiPin=31, clkPin=35):
        self.csPin = csPin
    ...
    def read(self):
        ...
        conf_reg = out[0]
        logger.debug("config register byte: %x" % conf_reg)
        ...
        logger.debug("high fault threshold: %d" % hft)

        [lft_msb, lft_lsb] = [out[5], out[6]]
        lft = ((lft_msb << 8) | lft_lsb) >> 1
        logger.debug("low fault threshold: %d" % lft)
    ...
    def calcPT100Temp(self, RTD_ADC_Code):
        ...
        logger.debug("RTD ADC Code: %d" % RTD_ADC_Code)
        Res_RTD = (RTD_ADC_Code * R_REF) / 32768.0   # PT100 Resistance
        logger.debug("PT100 Resistance: %f ohms" % Res_RTD)
        #
